...
This folder has the scripts used for generating the different synthetic data and running Demixer on them. If the number of samples are in hundreds (or < 100), then freebayes can also be run with -gvcf and --report-monomorphic options instead of variantcalling twice. Subsequently, positions without an alternate allele in any sample can be removed using bcftools with the MAF==0 filter.
