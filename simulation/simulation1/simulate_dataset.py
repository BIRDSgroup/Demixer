...
setargs.strain=''
no_run=10
rel_error=np.zeros((3,13,no_run))
current_directory = os.getcwd()
...
def topic_kldiverg(K,Docs_col_name,n_z_t,est_theta):
    ...
    for topic in range(0,K):
        ...
        for i in range(0,int(len(Docs_col_name)/4)):
                    ...
                    phi[i][topic][0]=[x for x in phi[i][topic][0]]
                    
                    divergence=divergence+np.array([entropy(phi[i][topic][0],np.around(t,4)) for t in sub])
...
for i in range(0,no_run):
    ...
    with open('Demixer_cpp/input/config.txt', 'w') as file:
        file.write(str(len(setargs.Docs)) + ' '+str(no_strain)+ ' '+str(setargs.V)+ ' '+str(setargs.V)+ ' '+str(setargs.V));
    ...
    print('running_NMF_SNP_LDA')
    
    output_to_file({})
    parallel_construct(False,'Demixer_cpp/input/',0,3,pd.DataFrame())

    runs(initial_dict,0,True);
    ...
    write_datfile("Demixer_cpp/input/idf.dat",setargs.idf)
    if(no_strain==3):
        beta_grid=[0.01,0.01,0.01,[0.01,1,1],[0.01,10,1]];
        alpha_grid=[1,[0.01,1,1],[0.01,10,1],0.01,0.01];
    
    if(no_strain==4):
        beta_grid=[0.01,0.01,0.01,[0.01,1,1,1],[0.01,10,1,1]]
        alpha_grid=[1,[0.01,1,1,1],[0.01,10,1,1],0.01,0.01];
    
    for j in range(0,5):
            setargs.alpha=alpha_grid[j]
            setargs.beta=beta_grid[j]
            if(isinstance(setargs.alpha, list)):
                alpha=np.array(setargs.alpha,dtype='<f4')
            else:
                alpha=np.full(setargs.K,setargs.alpha,dtype='<f4')
                
            if(isinstance(setargs.beta, list)):
                beta=np.array(setargs.beta,dtype='<f4')
            else:
                beta=np.full(setargs.K,setargs.beta,dtype='<f4')
            ...
            print('running_prior_NMF_SNP_LDA',setargs.alpha,setargs.beta)
            
            runs(variants_dict,1,True,True)
    ...
    with open('synthetic'+str(no_strain)+'strain/rel_error_'+str(setargs.K)+'strains.pkl', 'wb') as f:
